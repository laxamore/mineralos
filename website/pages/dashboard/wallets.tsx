      ...
      {isAuth ? (
        <>
          ...
          <div className="flex flex-col justify-center items-center h-full w-full">
            <ContentContext.Provider value={[setShowCreateWalletModal]}>
              <Content
                ...
                createButtonName={"Create New Wallet"}
                privilege={privilege}
              ...
              >
                <ul className="mt-2">
                  {rows.map((val: any) => {
                    return (
                      <li
                        className={`border border-gray-500 p-3 rounded-2xl cursor-pointer mt-2`}
                        key={val.original._id}
                      >
                        <div className="flex flex-row items-center">
                          <div className="w-1/6">
                            <p className="text-lg font-bold">
                              {val.original.wallet_name}
                            </p>
                          </div>
                          <div className="w-1/6">
                            <p className="text-lg font-bold">
                              ...
                              onClick={(e) => {
                                ...
                                withAuth(async (token: jwtObject) => {
                                  const response = await fetch(
                                    `${process.env.API_ENDPOINT}/api/v1/deleteWallet`,
                                    {
                                      method: "DELETE",
                                      mode: "cors",
                                      body: JSON.stringify({
          ...
          {showCreateWalletModal ? (
            <CreateWalletModal
              setShowModal={setShowCreateWalletModal}
              createWalletSuccessHandler={(res: never) => {
