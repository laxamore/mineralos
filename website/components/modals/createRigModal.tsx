...
const CreateRigModal: FC<Props> = ({
  setShowModal,
  createRigSuccessHandler,
    ...
    <Modal setShowModal={setShowModal}>
          ...
          if (data.rig_name != "") {
            withAuth(async (token: jwtObject) => {
              const response = await fetch(
                `${process.env.API_ENDPOINT}/api/v1/newRig`,
                {
                  method: "POST",
                  mode: "cors",
                  body: JSON.stringify({
      ...
      >
        <div className="flex flex-col mx-4 mb-4">
          <label htmlFor="rig_name" className="text-white font-semibold py-2">
            RIG Name
          </label>
