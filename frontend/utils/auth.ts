...
export function login(
  jwt: { jwt_token: string; jwt_token_expiry: number },
  noRedirect = false
) {
  inMemoryToken = {
...
export function checkAuth(ssr?: boolean, cookie?: string) {
  return new Promise(async (resolve) => {
    if (inMemoryToken.jwt_token_expiry * 1000 < Date.now() + 10000) {
      //   const cookiesExist = ssr ? cookie : doesHttpOnlyCookieExist("rtoken");
      ...
      const response = await fetch(
        `${
          ssr ? process.env.API_ENDPOINT_SSR : process.env.API_ENDPOINT
        }/api/v1/refreshToken`,
        options
      );
